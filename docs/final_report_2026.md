...
**Honesty analyses are co-primary (Component 1b).** With 21 susceptible patients the design is underpowered — ~21% power for even a 2.6-day effect, a ~9-day-wide interval. The one fix would be the **patient↔isolate re-linkage**, which we requested formally from Vivli; the data contributors declined to provide the linkage keys (18 Jun 2026, AMR Data Request #00013361), so the patient-level contrast is the analysis and the isolate file quantifies what the link *would* have bought (Component 1d): the mechanism-resolved phenotype the patient summary collapses is ascertained far more completely — **3GC-R in 204 isolates, MDR in 231, MRSA in 44** (the two Gram-negative mechanisms travel together, P(MDR\|3GC-R)=0.91) — and **MDR ascertainment alone would recover a 51-isolate susceptible arm against the patient frame's 21**, a ~1.5× tightening that would roughly halve the interval (~4.9 d) and lift power to ~55%. We ship the merge machinery and projection as a documented counterfactual (no positional join — the counts do not reconcile — and no workaround that would reconstruct the withheld keys). Crucially, the **exposure-ascertainment selection** that could most plausibly fake a result runs the *wrong way*: ascertained patients are longer-staying and lower-mortality, yet the excess is negative — surviving inverse-ascertainment reweighting (−1.85 d) and the missing-at-random anchor (−0.61 d).
...
**Burden–funding alignment (computed).** Pairing the verified GRAM-2019 per-pathogen burden (appendix Table S22, 95% UIs) against this funding, antibacterial R&D shows **no positive alignment with AMR mortality burden** across the six leading pathogens — Spearman ρ from **+0.14** (associated deaths) to **−0.49** (attributable DALYs), CI spanning ±1 at n=6. The floored per-pathogen log2 mismatch (Monte-Carlo over the burden UIs and the unfetched $113M Gram-negative split) is robust: **_S. pneumoniae_ (≈ +2.9) and _K. pneumoniae_ (≈ +2.0) are most under-funded** (together ≈99% of the "most under-funded" probability), while ***P. aeruginosa* (≈ −1.4) and _S. aureus_ (≈ −1.0) are over-funded** — the inverse alignment the systemic-leverage thesis predicts: money flows to a coordinated TB programme and to novel-compound targets, not to the highest-burden community Gram-negatives where access and empiric adequacy are the lever. This ranking is **corroborated by an independent funding source**: re-running the index against a live, directly-extracted Hub *by-genus* Data Table — filtered on the dashboard to match the locked denominator (public+philanthropic therapeutics, 2017–2023; total **$2.44bn / 128 funders ≈** the published **$2.51bn / 130**, the small residual being the Hub's documented retrospective revision between its 2026-06-30 live state and the 2026-01-09 freeze) — reproduces the inverse mismatch (Spearman ρ ≈ **−0.20**): the community Gram-negatives (*E. coli*, *K. pneumoniae*) stay under-funded and *P. aeruginosa* / *S. aureus* over-funded, and because the Therapeutics filter strips pneumococcal-*vaccine* R&D, the live extract **recovers the *S. pneumoniae* under-funding signal** (≈ +1.8, top of the ranking) — independently confirming the species-level primary rather than merely tracking it.
