...
> **Draft status.** Figures here are from the development run on the delivered files; the confirmatory run is executed in the Vivli secure environment. The R&D-Hub funding snapshot and the SPIDAAR codebook gates (exposure and adequacy coding) are now resolved; the one remaining Component-4 item is the exact per-pathogen log2 index, which awaits two appendix-only magnitude tables (noted inline). Every deviation from the pre-registration is logged in `docs/deviation_log.md` and a supplementary OSF addendum.
>
...
**Headline (verified).** Of US$2.51 billion in public+philanthropic antibacterial-therapeutics R&D (130 funders, 2017–2023), only **$1.06 billion — 42% — is pathogen-specific**; the majority (58%) is cross-cutting, so every per-pathogen claim is appropriately widened. Within the pathogen-specific minority, spend is dominated by *M. tuberculosis* ($474M, ~45% of it) and by Gram-positive/niche targets (*S. aureus* $142M, *C. difficile* $141M, *N. gonorrhoeae* $101M). The highest-AMR-burden Gram-negatives sit at the **bottom** of the funding ranking — *E. coli* (GRAM rank #1 for both associated and attributable deaths), *A. baumannii*, and *K. pneumoniae* all fall below *P. aeruginosa* ($87M) — and *S. pneumoniae* (GRAM rank #4) has **no dedicated species-specific funding at all**. This is the inverse alignment the systemic-leverage thesis predicts: money flows to a coordinated TB programme and to politically salient or novel-compound targets, not to the highest-burden community Gram-negatives where access and empiric adequacy are the lever. The exact per-pathogen log2 index and Spearman ρ await two appendix-only magnitude tables (Czaplewski appendix 1 p18; GRAM appendix per-pathogen deaths/DALYs) — the machinery is implemented and tested, and we deliberately do not hard-code figures we cannot quote. The argument it serves: if the systemic lever is access and empiric adequacy, R&D alignment should be read against *access and diagnostics*, not novel compounds alone.
