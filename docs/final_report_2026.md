...
**Abstract.** Across the highest-AMR-burden region on earth, the resistance *phenotype* is not the lever the burden headlines imply — and the system's resources are pointed away from where the lever actually is. In this sub-Saharan-African hospital cohort, as in the largest **adjusted** SSA/LMIC evidence, resistance is **not an independent per-patient killer and does not prolong stay** (competing-risks excess bed-days **−1.7 d**, interval crossing zero); the actionable burden is **systemic — whether a patient gets *adequate empiric therapy***, which we quantify with a g-formula and ship as a re-runnable stewardship simulator. And the system bends away from that burden on **three axes at once**: R&D **funding** and, we show here, disease **surveillance** are *both* misaligned with mortality burden — burden, funding, and surveillance all neglect the same **community Gram-negatives in sub-Saharan Africa** (the region carries the highest AMR death rate yet holds **2.3%** of global ATLAS isolates; the USA alone holds ~7× more). Two Cross-Domain components join the AMR Register to the Global AMR R&D Hub to make that triple mismatch explicit. The through-line is a **reallocation thesis**: in the highest-burden region, the levers we control — empiric-therapy access, R&D funding, and surveillance — should follow the burden they now bypass, and we ship the instruments to act on it.

> **Reproducibility.** Figures and headline numbers are produced by the committed pipeline on the delivered AMR Register / SPIDAAR files; a self-checking confirmatory harness (`scripts/confirmatory_run.py`) re-derives every headline value against a pinned manifest and reproduces them all (91/91 within tolerance). The R&D-Hub funding snapshot, the GRAM per-pathogen burden, and the SPIDAAR codebook gates (exposure and adequacy coding) are resolved and the Cross-Domain index is computed; the only un-fetched Component-4 input is the $113M split among three Gram-negatives (appendix-locked), propagated as uncertainty rather than guessed and not affecting the ranking. Every deviation from the pre-registration is logged in `docs/deviation_log.md` and a supplementary OSF addendum.
>
