...
The exposure is pinned to a defensible contrast — **resistant = `amrp==2`; susceptible = `amrp==0` only** (untested/unascertained excluded). All estimators are competing-risks-aware and reproducible (master seed `20260526`); because the secure environment lacks specialised survival/Bayesian libraries, they are implemented directly in NumPy/pandas and unit-tested on synthetic data (127 tests). Full specification: `docs/analysis_plan_2026.md`.
...
**A formal evidence synthesis, not narrative triangulation (Component 1c).** Rather than gesture at the literature, we pool the directly comparable, *adjusted* SSA/LMIC evidence on the resistance→in-hospital-death hazard ratio in a random-effects Bayesian meta-analysis (log-HR scale; weakly-informative priors; the per-study effect integrated out on a deterministic grid). MBIRA (adjusted cause-specific HR 0.74, 0.42–1.30) and Fiji (adjusted aHR 1.13, 0.51–2.53) pool to a **null in-hospital-death HR of 0.88 (95% credible interval 0.46–1.79; P(HR>1)=0.33)**, with a prediction interval for the next SSA setting of **0.30–2.77**. Our own cohort contributes only a *crude* cause-specific HR (2.76, 95% CI 0.37–20.8 — one susceptible death, severity-confounded); it is consistent in direction with the confounded crude signals the adjusted studies started from, and adding it barely moves the pool (0.95, 0.52–1.94). The synthesis makes the point precise: across the adjusted LMIC evidence, **resistance is not an independent per-patient mortality driver once severity and access are accounted for** — which is exactly why the headline contribution is the competing-risks excess-bed-days estimand, the dimension SPIDAAR uniquely supports, and not another underpowered mortality contrast.
...
**Hardened, not hyped.** A stratified bootstrap (resampling within each confounder×arm cell, so positivity is preserved by construction) puts honest intervals on these contrasts, and they are wide: pooled death-aversion +2.6 pp (95% CI −5.5 to +10.0), resistant arm +4.7 pp (−3.6 to +13.2), bed-days added 2.7 (−0.9 to +6.0). The direction is consistent but the cohort cannot exclude the null — which is precisely why the analysis is labelled exploratory rather than confirmatory. We also report an **E-value** for unmeasured confounding (VanderWeele & Ding 2017): an unmeasured confounder of both adequacy and death would need a risk-ratio association of **2.06 (pooled) / 3.03 (resistant arm)** to fully explain away the *point* death-aversion estimate — non-trivial — though only a weak one to reach the near-null confidence limit. The honest reading: the mechanism is clinically coherent and moderately robust to confounding at the point estimate, but conclusive proof needs the larger, re-linked cohort, not this one.
...
**Burden–funding alignment (computed).** Pairing the verified GRAM-2019 per-pathogen burden (appendix Table S22, with 95% UIs) against this funding, antibacterial R&D shows **no positive alignment with AMR mortality burden across the six leading pathogens** — Spearman ρ ranges from **+0.14** (associated deaths) to **−0.49** (attributable DALYs), with a CI spanning ±1 at n=6 (descriptive, no fitted line). The floored per-pathogen log2 mismatch (Monte-Carlo over the burden UIs and the one unfetched funding split) is robust: **_S. pneumoniae_ (log2 ≈ +2.9; ~17% of burden, no dedicated funding) and _K. pneumoniae_ (≈ +2.0) are the most under-funded relative to burden** — together carrying ≈99% of the "most under-funded" probability — while ***P. aeruginosa* (≈ −1.4, lowest burden but second-highest species funding) and _S. aureus_ (≈ −1.0) are over-funded.** This is the inverse alignment the systemic-leverage thesis predicts: money flows to a coordinated TB programme and to politically salient or novel-compound targets, not to the highest-burden community Gram-negatives where access and empiric adequacy are the lever. (The exact $113M *E. coli*/*A. baumannii*/*K. pneumoniae* split is appendix-locked and propagated as uncertainty, not hard-coded; it does not change the ranking.) The argument it serves: if the systemic lever is access and empiric adequacy, R&D alignment should be read against *access and diagnostics*, not novel compounds alone.

**Made local — the Cross-Domain finding in the catchment (computed).** The global GRAM ranking answers a global question; the Award asks what *our* data say. We re-weight the mismatch by the pathogen mix actually observed in the SPIDAAR severe-HAI isolates (the only delivered Register source spanning all six panel species — the ATLAS catchment cell is *E. coli*/*K. pneumoniae* only), using the resistant-isolate frequency as a local burden proxy (Dirichlet uncertainty over the counts; same global funding shares and propagated $113M split). The catchment lens **re-orders the priority**: the most under-funded relative to the *local* resistant-HAI burden are now ***K. pneumoniae* (log2 ≈ +2.5, carrying ≈93% of the "most under-funded" probability) and _E. coli_ (≈ +1.2, the single largest local share at ~37%)**, while *S. pneumoniae* — top of the *global* ranking — **flips to over-funded-relative (≈ −0.6) because it is nearly absent from SSA hospital infections (3 of 146 panel isolates)**. *S. aureus* (≈ −2.1) and *P. aeruginosa* (≈ −1.5) remain over-funded; the ranking is stable whether weighted by all isolates or resistant isolates only (Spearman ρ ≈ 0 either way — no alignment). The local picture sharpens the thesis: the pathogens that actually drive resistant hospital infection in the catchment are precisely the community Gram-negatives (*Klebsiella*, *E. coli*) that global R&D under-funds, not the globally-prominent pneumococcus. *(This is an isolation-frequency burden proxy, not a mortality burden, and is labelled as such.)*
...
Open-source pipeline (Apache-2.0, GitHub + Zenodo): SPIDAAR/ATLAS loaders, competing-risks excess-LOS + sensitivity + figure, Bayesian companion, the random-effects Bayesian evidence synthesis, ATLAS nowcast + frame-contrast, the global and catchment-specific R&D mismatch indices, the g-formula (with stratified-bootstrap CIs + an E-value), and the Streamlit tool; 127 unit tests on synthetic data; pre-registration + deviation log + OSF addendum. No stewardship recommendation is claimed final without local validation; the framework is built as infrastructure for catchment-region researchers, who have been engaged for independent review.

---

*References (separate page): Murray et al., Lancet 2022 (GRAM); Aiken et al., Lancet Infect Dis 2023 (MBIRA); Loftus et al., JGAR 2022 (Fiji); WHO-CHOICE bed-day unit costs (Stenberg et al. 2018); Czaplewski et al., Lancet Microbe 2026 (Global AMR R&D Hub funding 2017–23). Full verified figures in `docs/reference_verified_2026-06-06.md` and `docs/reference_rd_alignment_2026-06-09.md`.*
